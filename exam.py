...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    value  = [[999. for j in range(lRoad)] for i in range(nLanes)]
    policy = [[' ' for j in range(lRoad)] for i in range(nLanes)]
    change = True
            ...
            for j in range(lRoad):
                if goal[0] == i and goal[1] == j:
                    ...
                    for a in range(len(delta)):
                        ...
                        if i2 >= 0 and i2 < nLanes and j2 >= 0 and j2 < lRoad and road[i2][j2] != 0:
                            v2 = value[i2][j2] +  1. / road[i][j]
                            if a != 0:
                                v2 += lane_change_cost
                            if v2 < value[i][j]:
                                change = True
...
def show(lists, name=""):
    ...
    for i in range(len(lists)):
        if type(lists[i][0]) == float:
            print '[' + ', '.join(('%.3f'%j).rjust(7) for j in lists[i]) + ']'
        else:
            print '[' + ', '.join(str(j).rjust(7) for j in lists[i]) + ']'

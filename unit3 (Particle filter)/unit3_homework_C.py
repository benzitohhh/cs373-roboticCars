...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            error_bearing = abs(measurements[i] - predicted_measurements[i]) # i.e. 0<=e<=2pi
            error_bearing = (error_bearing + pi) % (2.0 * pi) - pi # truncate  i.e. -pi<=e<=pi
    ...
    def move(self, motion):
        # current orientation, coordinates
        ...
        if (beta >= 0.001):    
            ...
            # approximate as straight forward
            x = x + (d * cos(theta))
            y = y + (d * sin(theta))

        theta = (theta + beta) % (2*pi)

        # return new robot (aka particle)
        result = robot(l)
    ...
    def sense(self, applyNoise=1):
        ...
        for i in range(len(landmarks)):
            ...
            if (applyNoise != 0):
                beta += random.gauss(0, self.bearing_noise)
            beta %= (2*pi)
            Z.append(beta)
            
        return Z # vector of bearings

...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        mapping = [0, 1] + range(4, dim + 2)
        Omega   = Omega.expand(dim + 2, dim + 2, mapping, mapping)
        Xi      = Xi.expand(dim + 2, 1, mapping, [0])
...
answer_omega2      = matrix([[0.22871751620895048, 0.0, -0.11351536555795691, 0.0, -0.11351536555795691, 0.0],
                             ...
                             [0.0, -0.1811320754716981536555795691, 0.0, -0.46327947920510265, 0.0, 0.7867205207948973]])
